    ...
    django.stdout.on('data', function(data){
        ...
        if(~data.indexOf('Quit the server with CONTROL-C.')){
            ...
            i.question('Press '+'[Enter]'.blue.bold+' to launch your browser.', function(answer){
                ...
                if (is_sudo) {
                    exec('su '+process.env.SUDO_USER+' -c \'open "http://dev.django.org:8000/"\'');
                } else {
                    exec('open "http://dev.django.org:8000/"');
                }
                console.log();
...
var resolveCacheFile = process.cwd() + '/.django_resolve_cache.json';
try {
    resolveCache = require(resolveCacheFile);
} catch(e) {
    fs.writeFileSync(resolveCacheFile, '{}');
    if (is_sudo) {
        fs.chownSync(resolveCacheFile, ~~process.env.SUDO_UID, ~~process.env.SUDO_GID);
    }
}

function resolve(path, cb) {
    if ( path in resolveCache ) {
        return cb(resolveCache[path].stderr, resolveCache[path].stdout);
    }
