...
function django_resolver(files, cb) {
    // just resolve one for now to be lazy
    var file = files[0];
    exec(options.manage_py + ' findstatic --first '+path, function(err, stdout, stderr) {
        if(stdout) {
            stderr = 0;
            ...
            for (; i < lines.length; i++) {
                if (/^Found /.test(lines[i])) {
                    ...
                    stdout = [{file: path.basename(stdout), root: path.dirname(stdout)}];
                    break;
                }
            }
        } else {
